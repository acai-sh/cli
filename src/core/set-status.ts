import type { FeatureStatesRequest, FeatureStatesResponse } from "../generated/types.ts";
...
const VALID_STATUSES = new Set(["assigned", "blocked", "incomplete", "completed", "rejected", "accepted"]);
const FULL_ACID_PATTERN = /^[A-Za-z0-9_-]+\.[A-Z][A-Z0-9_-]*\.[0-9]+(?:-[0-9]+)?$/;
...
export interface SetStatusDependencies extends OneImplementationResolverDependencies {
...
export function parseFeatureStatesPayload(jsonText: string): ParsedFeatureStatesPayload {
  let parsed: unknown;
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw usageError("Status payload must be a JSON object keyed by full ACID.");
  }
  ...
  for (const acid of acids) {
    ...
    if (featureName && featureName !== acidFeatureName) {
      throw usageError("All ACIDs in one payload must share the same feature prefix.");
    }
