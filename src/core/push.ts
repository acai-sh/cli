...
export type PushReference = NonNullable<PushRequest["references"]>["data"][string][number];
...
const IGNORED_REF_DIRS = new Set([".git", "node_modules", "coverage", "dist", "tmp", ".agents", "states"]);
const TEST_PATH_SEGMENTS = new Set(["test", "tests", "__tests__"]);
const REF_SCAN_EXCLUDED_SUFFIXES = new Set([".png", ".jpg", ".jpeg", ".gif", ".ico", ".pdf", ".zip", ".gz", ".woff", ".woff2", ".ttf", ".otf", ".wasm", ".bin"]);
const FULL_ACID_PATTERN = /\b([A-Za-z0-9_-]+\.[A-Z][A-Z0-9_-]*\.[0-9]+(?:-[0-9]+)?)\b/g;

// push.MAIN.7 / push.MAIN.8 / push.SCAN.3 / push.SAFETY.2
export async function planPush(options: PushCommandOptions = {}): Promise<PushPlan> {
  ...
  const [gitContext, scan] = await Promise.all([
    ...
    scanPushRepo({ cwd: repoRoot, runner, featureNames: options.featureNames, repoRoot }),
  ]);
...
export async function scanPushRepo(options: {
  cwd?: string;
  runner?: GitCommandRunner;
  featureNames?: string[];
  repoRoot?: string;
  ...
  for (const relativePath of filePaths) {
    ...
    if (featureFilter !== undefined && !featureFilter.has(spec.spec.feature.name)) continue;

    specs.push(spec);
  }

  const references = await scanPushReferences(repoRoot, filePaths, featureFilter);
...
export async function scanPushReferences(
  ...
  for (const relativePath of filePaths) {
    ...
    for (const match of text.matchAll(FULL_ACID_PATTERN)) {
      ...
      if (featureFilter !== undefined && !featureFilter.has(featureName)) continue;
...
function getLineNumberForMatch(text: string, index: number | undefined): number {
...
export function normalizePushOptions(options: PushCommandOptions): NormalizedPushOptions {
...
export async function runPushCommand(
  ...
  const resolvedPlan = plan ?? (await planPush({
  ...
  const settledResults: PromiseSettledResult<Awaited<ReturnType<ApiClient["push"]>>>[] = [];
  ...
  settledResults.forEach((result, index) => {
    ...
    if (result.status === "fulfilled") {
      ...
      successes.push({
        productName: data.product_name ?? payloadForIndex.product_name ?? "unknown-product",
    ...
    failures.push({
      ...
      error: result.reason instanceof Error ? result.reason.message : "Push request failed.",
    });
  });
  ...
  const diagnostics = failures.map((failure) => `Push failed for ${failure.productName}: ${failure.error}`);

  return {
    exitCode: failures.length > 0 ? 1 : 0,
    stdoutLines: textLines,
    stderrLines: options.json ? diagnostics.concat(payload.results.flatMap((result) => result.warnings.map((warning) => `Warning for ${result.productName}: ${warning}`))) : [],
...
function aggregatePushSuccesses(successes: PushSuccessSummary[]): PushSuccessSummary[] {
  const merged = new Map<string, PushSuccessSummary>();

  for (const success of successes) {
    const existing = merged.get(success.productName);
    if (!existing) {
      merged.set(success.productName, { ...success, warnings: [...success.warnings] });
      continue;
    }

    existing.implementationName = existing.implementationName ?? success.implementationName;
  ...
  return [...merged.values()].sort((left, right) => left.productName.localeCompare(right.productName));
}

function countPayloadReferences(payload: PushRequest): number {
  if (!payload.references?.data) return 0;
  return Object.values(payload.references.data).reduce((total, refs) => total + refs.length, 0);
}

function aggregatePushFailures(failures: PushFailureSummary[]): PushFailureSummary[] {
  const merged = new Map<string, AggregatedPushFailure>();

  for (const failure of failures) {
    const existing = merged.get(failure.productName);
    if (!existing) {
      merged.set(failure.productName, { productName: failure.productName, errors: [failure.error] });
      continue;
    }
...
export function parseFeatureDocument(raw: string, relativePath: string): ParsedFeatureDocument {
  ...
  const feature = asRecord(root.feature, `Invalid feature spec: ${relativePath}`);
  const featureName = asString(feature.name, `Invalid feature spec: ${relativePath}`);
  const productName = asString(feature.product, `Invalid feature spec: ${relativePath}`);

  const spec: PushSpec = {
    feature: {
      name: featureName,
      product: productName,
      version: typeof feature.version === "string" && feature.version.trim() ? feature.version : "1.0.0",
      ...(typeof feature.description === "string" ? { description: feature.description } : {}),
      ...(Array.isArray(feature.prerequisites)
        ? { prerequisites: feature.prerequisites.map((value) => asString(value, `Invalid feature spec: ${relativePath}`)) }
        : {}),
    },
    meta: {
      last_seen_commit: "",
  ...
  for (const sectionName of ["components", "constraints"]) {
    ...
    for (const [componentName, componentValue] of Object.entries(section as Record<string, unknown>)) {
      const component = asRecord(componentValue, `Invalid feature spec: ${relativePath}`);
      const requirementValues = asRecord(component.requirements, `Invalid feature spec: ${relativePath}`);
      const componentDeprecated = component.deprecated === true;
      const notes = collectNotes(requirementValues);

      for (const [requirementId, requirementValue] of Object.entries(requirementValues)) {
        if (requirementId.endsWith("-note")) continue;

        const requirement = asRequirementDefinition(requirementValue, `Invalid feature spec: ${relativePath}`);
        ...
        const entry: PushSpec["requirements"][string] = {
          ...
          ...(requirement.note ?? notes.get(requirementId) ? { note: requirement.note ?? notes.get(requirementId) } : {}),
          ...(requirement.replaced_by ? { replaced_by: requirement.replaced_by } : {}),
        };
  ...
  const sortedRequirements = [...requirements.entries()].sort(([left], [right]) => left.localeCompare(right));
...
export function buildPushPayloads(
  ...
  const featureFilter = normalizeFeatureFilter(options.featureNames);
  const specs = featureFilter ? scan.specs.filter((entry) => featureFilter.has(entry.featureName)) : [...scan.specs];
  const references = featureFilter ? scan.references.filter((entry) => featureFilter.has(entry.featureName)) : [...scan.references];

  const featureToProducts = new Map<string, Set<string>>();
  for (const spec of specs) {
    const products = featureToProducts.get(spec.featureName) ?? new Set<string>();
  ...
  for (const productName of [...buckets.keys()].sort((left, right) => left.localeCompare(right))) {
    ...
    const payload: PushRequest = {
      ...
      ...(bucket.references.size > 0 ? { references: { data: sortReferenceMap(bucket.references), override: false } } : {}),
      ...(productName && resolveScopedSelector(options.target, productName) ? { target_impl_name: resolveScopedSelector(options.target, productName) } : {}),
      ...(productName && resolveScopedSelector(options.parent, productName) ? { parent_impl_name: resolveScopedSelector(options.parent, productName) } : {}),
    };
...
async function walkFiles(root: string, directory: string): Promise<string[]> {
  ...
  while (queue.length > 0) {
    ...
    for (const entry of dirEntries) {
      ...
      const relativePath = relative(root, join(current, entry.name)).split("\\").join("/");
...
function parseFeatureDocumentInternal(raw: string, relativePath: string): ParsedFeatureDocument {
  const document = loadYaml(raw);
  if (!document || typeof document !== "object") {
    throw runtimeError(`Invalid feature spec: ${relativePath}`);
  }
...
function collectNotes(requirementValues: Record<string, unknown>): Map<string, string> {
...
function asRequirementDefinition(
  value: unknown,
  message: string,
): { requirement: string; deprecated?: boolean; note?: string; replaced_by?: string[] } {
  ...
  return {
    ...
    ...(Array.isArray(record.replaced_by)
      ? { replaced_by: record.replaced_by.map((entry) => asString(entry, message)) }
      : Array.isArray(record.replacements)
        ? { replaced_by: record.replacements.map((entry) => asString(entry, message)) }
        : {}),
  };
}

function isFeatureSpecPath(relativePath: string): boolean {
  return relativePath.startsWith(FEATURE_SPEC_PREFIX) && relativePath.endsWith(FEATURE_SPEC_SUFFIX);
}

function shouldScanForReferences(relativePath: string): boolean {
  if (isFeatureSpecPath(relativePath)) return false;
  if (relativePath.split("/").some((segment) => IGNORED_REF_DIRS.has(segment))) return false;

  const lower = relativePath.toLowerCase();
  return ![...REF_SCAN_EXCLUDED_SUFFIXES].some((suffix) => lower.endsWith(suffix));
}
...
function compareReferenceEntries(left: DiscoveredPushReference, right: DiscoveredPushReference): number {
  return (
...
function pushReferenceToBucket(bucket: ProductBucket, reference: DiscoveredPushReference): void {
...
function sortReferenceMap(referenceMap: Map<string, PushReference[]>): Record<string, PushReference[]> {
  const sortedEntries = [...referenceMap.entries()].sort(([left], [right]) => left.localeCompare(right));
  const result: Record<string, PushReference[]> = {};

  for (const [acid, refs] of sortedEntries) {
    result[acid] = [...refs].sort((left, right) => left.path.localeCompare(right.path) || Number(left.is_test) - Number(right.is_test));
  }

  return result;
}

function resolveScopedSelector(value: string | undefined, productName: string): string | undefined {
...
function normalizeFeatureFilter(featureNames?: string[]): Set<string> | undefined {
  const names = featureNames?.map((entry) => entry.trim()).filter((entry): entry is string => entry.length > 0);
