...
export function normalizeOneImplementationTarget(
  ...
  if (options.product && scopedTarget && options.product !== scopedTarget.productName) {
    throw usageError(
  ...
  if (!productName) {
    throw usageError("Missing product selector. Provide --product or use --impl <product/implementation>.");
  }
...
export function parseScopedImplementationSelector(selector?: string): { productName: string; implementationName: string } | undefined {
  if (!selector) {
    return undefined;
  }
