...
describe("skill.MAIN.2 skill.MAIN.3 skill.SAFETY.3 skill.UX.1", () => {
  test("skill.MAIN.2 loads the canonical bundled skill markdown verbatim", async () => {
    const canonicalFile = await readFile(new URL("../.agents/skills/acai/SKILL.md", import.meta.url), "utf8");
...
describe("skill.MAIN.4 skill.WRITE.1 skill.WRITE.2 skill.WRITE.3 skill.SAFETY.3", () => {
  ...
  test("skill.WRITE.1 resolves the install path under the caller cwd", () => {
    expect(resolveSkillInstallPath("/tmp/example")).toBe("/tmp/example/.agents/skills/acai/SKILL.md");
  });
...
describe("cli-core.DIST.1 cli-core.DIST.2 cli-core.DIST.3 skill.MAIN.2 skill.MAIN.3", () => {
  test("cli-core.DIST.1 bundles the canonical skill content into the node distribution", async () => {
    ...
    try {
      const build = await runProcess(
        ["bun", "build", "./src/index.ts", "--target=node", "--outfile", bundlePath],
        workspaceRoot,
      );

      expect(build.exitCode).toBe(0);

      const result = await runProcess(["node", bundlePath, "skill"], outputRoot);
  ...
  test("cli-core.DIST.2 cli-core.DIST.3 compile a runnable binary that prints the canonical skill content", async () => {
    ...
    try {
      const build = await runProcess(
        ["bun", "build", "./src/index.ts", "--compile", "--outfile", binaryPath],
        workspaceRoot,
      );
